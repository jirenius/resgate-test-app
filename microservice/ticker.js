...
nats.subscribe('call.tickerService.ticker.set', (request, replyTo, subject) => {
	...
	if (Object.keys(change).length) {
		...
		nats.publish('event.tickerService.ticker.change', JSON.stringify({ values: change }));
	}
...
let count = function() {
	setTimeout(() => {
		ticker.count++;
		nats.publish('event.tickerService.ticker.change', JSON.stringify({
			values: { count: ticker.count }
		}));
		count();
	}, 1000);
};
